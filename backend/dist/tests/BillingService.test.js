...
describe('BillingService', () => {
    ...
    describe('calculateFee', () => {
        ...
        it('Should return 3000 VND if the vehicle is picked up on a Sunday (regardless of the time)', async () => {
            ...
            PricingPolicy_1.PricingPolicy.findOne.mockResolvedValue(mockPolicy);
            const sundayDate = new Date('2026-05-10T10:00:00');
            const fee = await BillingService_1.BillingService.calculateFee(sundayDate, 'MOTORBIKE');
            expect(fee).toBe(3000);
            expect(PricingPolicy_1.PricingPolicy.findOne).toHaveBeenCalledTimes(1);
        });
        it('Should return 3000 VND if the vehicle is picked up from 18:00 onwards (Monday - Saturday)', async () => {
            ...
            const fee = await BillingService_1.BillingService.calculateFee(mondayNightDate, 'MOTORBIKE');
            expect(fee).toBe(3000);
        });
        it('Should return 2000 VND if the vehicle is picked up before 18:00 (Monday - Saturday)', async () => {
            ...
            const fee = await BillingService_1.BillingService.calculateFee(mondayAfternoonDate, 'MOTORBIKE');
            expect(fee).toBe(2000);
        });
        it('Should throw an error if no ACTIVE PricingPolicy is found', async () => {
            PricingPolicy_1.PricingPolicy.findOne.mockResolvedValue(null);
            const someDate = new Date('2026-05-11T15:00:00');
            await expect(BillingService_1.BillingService.calculateFee(someDate, 'MOTORBIKE')).rejects.toThrow();
        });
    });
    describe('calculateCycleFee', () => {
        it('Should calculate the total fee accurately for 100 users, with each user having 30-35 sessions', () => {
            ...
            mockUsers.forEach(userId => {
                ...
                const calculatedTotal = BillingService_1.BillingService.calculateCycleFee(mockSessions);
        ...
        it('Should accurately accumulate the total from the fee fields', () => {
            ...
            const total = BillingService_1.BillingService.calculateCycleFee(mockSessions);
            expect(total).toBe(7000);
        });
        it('Should return 0 if the array is empty', () => {
            const total = BillingService_1.BillingService.calculateCycleFee([]);
            expect(total).toBe(0);
        });
    });
});
